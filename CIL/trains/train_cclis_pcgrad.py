...
def adjust_learning_rate_cclis_pcgrad(args, optimizer, epoch):
    ...
    for idx, param_group in enumerate(optimizer._optim.param_groups):
...
def train_cclis_pcgrad(opt, model, model2, criterion, optimizer, scheduler, train_loader, epoch, subset_sample_num, score_mask, grad_train_loaders, grad_val_loaders,
    ...
    for idx, (images, labels, importance_weight, index) in enumerate(train_loader):
        ...
        # 現在タスクと過去タスクを分割するための指標
        target_start = opt.target_task * opt.cls_per_task
        target_end   = (opt.target_task + 1) * opt.cls_per_task - 1
        ...
        #                  reduction='mean')
        # print("loss: ", loss)     # loss:  tensor(6.2404, device='cuda:0', dtype=torch.float64, grad_fn=<DivBackward0>)
        ...
        # # print("loss.shape: ", loss.shape)    # loss.shape:  torch.Size([512])
        # print("loss.sum(): ", loss.sum())    # loss.sum():  tensor(6.2404, device='cuda:0', dtype=torch.float64, grad_fn=<SumBackward0>)
        # assert False
        ...
        # 現在タスクかどうかで区分け（合計ベース）
        loss_prev_sum = torch.zeros((), device=loss.device, dtype=loss.dtype)
        loss_curr_sum = torch.zeros((), device=loss.device, dtype=loss.dtype)
        prev_cnt = 0
        curr_cnt = 0

        for c, idxs in label_to_indices.items():
            if c < target_start:
                loss_prev_sum = loss_prev_sum + loss[idxs].sum()
                prev_cnt += len(idxs)
            elif c <= target_end:
                loss_curr_sum = loss_curr_sum + loss[idxs].sum()
                curr_cnt += len(idxs)
        ...
        if distill_type == 'IRD':
            ...
            if opt.target_task > 0:
                ...
                loss_distill = (-logits2 * torch.log(logits1)).sum(0)  # shape: [batch_size]

                # 旧タスク（target_start 未満）／現タスク（その範囲内）
                prev_mask = labels < target_start
                curr_mask = (labels >= target_start) & (labels <= target_end)


                zero = loss_distill.sum() * 0.0

                # 分割して平均（バッチ構成の偏りを避けたいので mean 推奨。総和にしたいなら .sum()）
                loss_prd_prev = loss_distill[prev_mask].sum() / len(labels) if prev_mask.any() else zero
                loss_prd_curr = loss_distill[curr_mask].sum() / len(labels) if curr_mask.any() else zero

                # loss += opt.distill_power * loss_distill
                distill.update(loss_distill.mean().item(), bsz)
        else:
        ...
        if opt.target_task == 0:
            loss = loss_curr_sum
            objectives = [loss_curr_sum]
        else:
            loss = loss_curr_sum + loss_curr_sum + opt.distill_power * loss_prd_curr + opt.distill_power * loss_prd_prev
            objectives = [loss_curr_sum, loss_curr_sum, opt.distill_power * loss_prd_curr, opt.distill_power * loss_prd_prev]
        ...
        # 現在の学習率
        current_lr = optimizer._optim.param_groups[0]['lr']
...
def grad_analysis_distill(opt, model, model2, optimizer, criterion, grad_loader, epoch, distill_type):
    ...
    for _, (images, labels, importance_weight, index) in tqdm(enumerate(grad_loader)):
        ...
        loss = (-logits2 * torch.log(logits1)).sum(0)  # shape: [batch_size]
