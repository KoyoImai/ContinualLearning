...
def rfr_loss(features, eps=1e-8):

    N, d = features.shape
    # print("features.shape: ", features.shape)

    # mean-centering（これがないと共分散の意味が崩れる）
    features_centered = features - features.mean(dim=0, keepdim=True)

    # 共分散行列 C = (1/N) * H^T H
    cov = torch.matmul(features_centered.T, features_centered) / N  # shape: (d, d)
    # print("cov.shape: ", cov.shape)

    # SVD: H = UΣV^T, s: 特異値ベクトル（shape: min(N, d)）
    _, s, _ = torch.svd(cov, some=False)
    ...
    # 
    logged_erank = - (s_squared * torch.log(s_squared)).nansum()
...
def train_cclis_rfr(opt, model, model2, criterion, optimizer, scheduler, train_loader, epoch, subset_sample_num, score_mask):
    ...
    for idx, (images, labels, importance_weight, index) in enumerate(train_loader):
        ...
        # rfr loss の計算
        if opt.target_task == 0:
            loss_rfr = rfr_loss(features=features)
            loss += opt.rfr_power * loss_rfr
        else:
            with torch.no_grad():
                loss_rfr = rfr_loss(features=features)
        ...
        if (idx+1) % opt.print_freq == 0 or idx+1 == len(train_loader):
            print('Train: [{0}][{1}/{2}]\t'
                  ...
                  'loss_rfr {loss_rfr:3f}\t'
                  'lr {lr:.5f}'.format(
                   epoch, idx + 1, len(train_loader), loss=losses, loss_rfr=loss_rfr, lr=current_lr))
