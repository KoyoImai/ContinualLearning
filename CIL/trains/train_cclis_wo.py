...
def adjust_learning_rate_cclis_wo(args, optimizer, epoch):
...
def val_cclis_wo(opt, model, model2, linear_loader, val_loader, taskil_loaders, epoch):
    ...
    # 検証（これまで学習した各タスク毎に）
    all_task_accuracies, all_task_losses = taskil_val_cclis_wo(opt, model, classifier, criterion, taskil_loaders)
...
def taskil_val_cclis_wo(opt, model, classifier,  criterion, val_loaders):
...
def ncm_cclis_wo(model, ncm_loader, val_loader):
