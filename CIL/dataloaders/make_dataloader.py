...
# 勾配解析用
from dataloaders.dataloader_co2l import set_grad_loader_co2l_cifar10, set_grad_loader_co2l_cifar100, set_grad_loader_co2l_tinyimagenet
from dataloaders.dataloader_co2l import set_gradtask_loader_co2l_cifar10, set_gradtask_loader_co2l_cifar100, set_gradtask_loader_co2l_tinyimagenet
from dataloaders.dataloader_co2l import set_grad_loader_co2l_cifar10_v2, set_grad_loader_co2l_cifar100_v2, set_grad_loader_co2l_tinyimagenet_v2
from dataloaders.dataloader_co2l import set_gradreplay_loader_co2l_cifar10, set_gradreplay_loader_co2l_cifar100, set_gradreplay_loader_co2l_tinyimagenet
...
from dataloaders.dataloader_cclis import set_gradreplay_loader_cclis_cifar10, set_gradreplay_loader_cclis_cifar100, set_gradreplay_loader_cclis_tinyimagenet
...
# joint
from dataloaders.dataloader_joint import set_loader_joint_cifar10, set_valloader_joint_cifar10, set_linearloader_joint_cifar10
...
def set_loader(opt, replay_indices, method_tools):
    ...
    if opt.method == "er":
    ...
    elif opt.method in ["cclis", "cclis-wo", "cclis-wo-is", "cclis-rfr", "cclis-grad", "cclis-pcgrad"]:
        ...
        if opt.dataset == "cifar10":
            train_loader, subset_indices = set_loader_joint_cifar10(opt=opt, normalize=normalize)
            val_loader = set_valloader_joint_cifar10(opt=opt, normalize=normalize)
            linear_loader = set_linearloader_joint_cifar10(opt=opt, normalize=normalize)
        elif opt.dataset == "cifar100":
            train_loader, subset_indices = set_loader_joint_cifar100(opt=opt, normalize=normalize)
            val_loader = set_valloader_joint_cifar100(opt=opt, normalize=normalize)
            linear_loader = set_linearloader_joint_cifar100(opt=opt, normalize=normalize)
        elif opt.dataset == 'tiny-imagenet':
            train_loader, subset_indices = set_loader_joint_tinyimagenet(opt=opt, normalize=normalize)
            val_loader = set_valloader_joint_tinyimagenet(opt=opt, normalize=normalize)
            linear_loader = set_linearloader_joint_tinyimagenet(opt=opt, normalize=normalize)

    elif opt.method in ["cclis-wo-ss"]:

        if opt.dataset == "cifar10":
            train_loader, subset_indices, subset_sample_num = set_loader_cclis_wo_ss_cifar10(opt=opt, normalize=normalize, replay_indices=replay_indices, method_tools=method_tools)
            post_loader, _, _ = set_loader_cclis_cifar10(opt=opt, normalize=normalize, replay_indices=replay_indices, method_tools=method_tools, training=False)
            val_loader = set_valloader_co2l_cifar10(opt=opt, normalize=normalize)
            linear_loader = set_linearloader_co2l_cifar10(opt=opt, normalize=normalize, replay_indices=replay_indices)
        elif opt.dataset == "cifar100":
            train_loader, subset_indices, subset_sample_num = set_loader_cclis_wo_ss_cifar100(opt=opt, normalize=normalize, replay_indices=replay_indices, method_tools=method_tools)
            post_loader, _, _ = set_loader_cclis_cifar100(opt=opt, normalize=normalize, replay_indices=replay_indices, method_tools=method_tools, training=False)
            val_loader = set_valloader_co2l_cifar100(opt=opt, normalize=normalize)
            linear_loader = set_linearloader_co2l_cifar100(opt=opt, normalize=normalize, replay_indices=replay_indices)
        elif opt.dataset == 'tiny-imagenet':
    ...
    elif opt.method in ["co2l", "supcon", "supcon-joint"]:

        if opt.dataset == "cifar10":
            grad_val_loaders = set_grad_loader_co2l_cifar10_v2(opt=opt, train=False, normalize=normalize)
        elif opt.dataset == "cifar100": 
            grad_val_loaders = set_grad_loader_co2l_cifar100_v2(opt=opt, train=False, normalize=normalize)
        elif opt.dataset == "tiny-imagenet":
            grad_val_loaders = set_grad_loader_co2l_tinyimagenet_v2(opt=opt, train=False, normalize=normalize)

        if opt.dataset == "cifar10":
            grad_train_loaders = set_grad_loader_co2l_cifar10_v2(opt=opt, train=True, normalize=normalize)
        elif opt.dataset == "cifar100":
            grad_train_loaders = set_grad_loader_co2l_cifar100_v2(opt=opt, train=True, normalize=normalize)
        elif opt.dataset == "tiny-imagenet":
            grad_train_loaders = set_grad_loader_co2l_tinyimagenet_v2(opt=opt, train=True, normalize=normalize)
    
    elif opt.method in ["cclis", "cclis-pcgrad"]:
    ...
    if opt.method in ["er"]:
        ...
        if opt.dataset == "cifar10":
            ...
            # grad_val_loaders = set_gradtask_loader_co2l_cifar100(opt=opt, train=False, normalize=normalize)
            grad_val_loaders = None
        elif opt.dataset == "tiny-imagenet":
            # grad_val_loaders = set_gradtask_loader_co2l_tinyimagenet(opt=opt, train=False, normalize=normalize)
            grad_val_loaders = None

        if opt.dataset == "cifar10":
            # grad_train_loaders = set_gradtask_loader_co2l_cifar10(opt=opt, train=True, normalize=normalize)
            grad_train_loaders = None
        elif opt.dataset == "cifar100":
    ...
    if opt.method in ["er"]:
        ...
        if opt.dataset == "cifar10":
            gradreplay_val_loader = set_gradreplay_loader_cclis_cifar10(opt=opt, training=False, normalize=normalize, replay_indices=[], method_tools=method_tools)
        elif opt.dataset == "cifar100": 
            gradreplay_val_loader = set_gradreplay_loader_cclis_cifar100(opt=opt, training=False, normalize=normalize, replay_indices=[], method_tools=method_tools)
        elif opt.dataset == "tiny-imagenet":
            gradreplay_val_loader = set_gradreplay_loader_cclis_tinyimagenet(opt=opt, training=False, normalize=normalize, replay_indices=[], method_tools=method_tools)

        if opt.dataset == "cifar10":
            gradreplay_train_loader = set_gradreplay_loader_cclis_cifar10(opt=opt, training=True, normalize=normalize, replay_indices=replay_indices, method_tools=method_tools)
        elif opt.dataset == "cifar100":
            gradreplay_train_loader = set_gradreplay_loader_cclis_cifar100(opt=opt, training=True, normalize=normalize, replay_indices=replay_indices, method_tools=method_tools)
        elif opt.dataset == "tiny-imagenet":
            gradreplay_train_loader = set_gradreplay_loader_cclis_tinyimagenet(opt=opt, training=True, normalize=normalize, replay_indices=replay_indices, method_tools=method_tools)
