...
class BarlowTwinsLoss(nn.Module):
    def __init__(self, opt=None, size=128):
        ...
        self.bn = nn.BatchNorm1d(size, affine=False)
    ...
    #     # c.div_(self.opt.batch_size)
    #     # torch.distributed.all_reduce(c)

    #     on_diag = torch.diagonal(c).add_(-1).pow_(2).sum()
    #     off_diag = off_diagonal(c).pow_(2).sum()

    #     # loss = on_diag + self.opt.bw_lambd * off_diag
    #     loss = self.opt.bw_lambd * off_diag
    ...
    def forward(self, z1, z2):
        ...
        # # loss
        # c_diff = (c - torch.eye(D, device=torch.device('cuda'))).pow(2) # DxD
        
        # # multiply off-diagonal elems of c_diff by lambda
        # c_diff[~torch.eye(D, dtype=bool)] *= self.opt.bw_lambd
        # loss = c_diff.sum()

        # create diagonal and off-diagonal masks
        eye = torch.eye(D, device=c.device, dtype=torch.bool)
        not_eye = ~eye

        # on-diagonal term: (C_ii - 1)^2
        on_diag = ((c[eye] - 1) ** 2).sum()
        # print("on_diag: ", on_diag)

        # off-diagonal term: C_ij^2 (i ≠ j)
        off_diag = (c[not_eye] ** 2).sum()
        # print("off_diag: ", off_diag)

        # total loss
        # loss = on_diag + self.opt.bw_lambd * off_diag
        loss = off_diag * self.opt.bw_lambd
