...
def parse_option():
    ...
    opt.save_path = f'./logs/{opt.method}/{opt.log_name}/annalyze/scorewise/offline_{opt.offline}'
...
def main():
    ...
    for target_task in range(0, opt.n_task):
        ...
        task_loaders = data_loaders["task"]

        for idx, dataloader in enumerate(task_loaders):
            
            # 特徴量を抽出（これまでのタスクに含まれるすべてのサンプル）
            features, labels, layer_outputs = extract_features(opt=opt, model=model, data_loader=dataloader)
            # print("1 features.shape: ", features.shape)

            # スコア計算用インスタンス
            agent_score = Eval_cluster(model=model, dataloader=dataloader, target_module=None, num_features=None)
